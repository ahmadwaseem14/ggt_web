...
function myFunction() {
  if ($(window).width() > 1199) {
    if ($(window).scrollTop() > 145) {
      ...
      $('#siteNav.medium > li > a').css('color', 'black'); // Change direct <a> color
      $('.anm-bag-l').css("color", "black"); // Change color for the icon/text inside
      $('.anm-bag-l').addClass("black-icon"); // Add class to style the ::before element
    } else {
      $('.header-wrap').removeClass("stickyNav fadeInDown");
      $('#siteNav.medium > li > a').css('color', 'white'); // Revert color
      $('.anm-bag-l').css("color", "white"); // Revert color for the icon/text inside
      $('.anm-bag-l').removeClass("black-icon"); // Remove class
    }
  }
}
	...
	$(document).ready(function() {
		...
		$('.search-bar__form').on('submit', function() {
			if ($('.search__input').val().trim()) {
				$('.search__button').html('<i class="fa fa-spinner fa-spin"></i>');
			}
		});
	});
